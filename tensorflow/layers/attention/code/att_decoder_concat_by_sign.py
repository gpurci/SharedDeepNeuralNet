...
from attention.code.att_feed_forward_concat_by_sign import *

class AttDecoderConcatBySign(layers.Layer):
    def __init__(self, embed_dim, num_heads, size_dff, dropout_rate=0.0, **kw):
        ...
        super(AttDecoderConcatBySign, self).__init__(**kw)
    ...
    def __init_from_config(self, kw):
        ...
        if (config is not None):
            self.att_ffn = AttFeedForwardConcatBySign(**config)
        else:
            self.att_ffn = AttFeedForwardConcatBySign(
                                        embed_dim=self.embed_dim, 
                                        size_dff=self.size_dff, 
                                        dropout_rate=self.dropout_rate, 
                                        name="AttFFN")
