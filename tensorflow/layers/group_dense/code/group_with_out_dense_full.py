...
class GroupWithOutDenseFull(layers.Layer):
   def __init__(self, windows_size, out_window_size, activation=None, **kw):
      super(GroupWithOutDenseFull, self).__init__(**kw)
   ...
   def _build_shape(self, input_shape):
      ...
      shape    = (*input_shape[1:-1], seq_size, self.windows_size)
      self.l_reshape_in  = layers.Reshape(shape, name="reshape_in")
      shape    = (*input_shape[1:-1], seq_size*self.out_window_size)
   ...
   def _build_kernel_seq(self, channels_size):
      ...
      w = self.add_weight(shape=(seq_size, self.windows_size, self.out_window_size),
