...
class GroupDenseUnits(layers.Layer):
   def __init__(self, units, out_units, activation=None, **kw):
      super(GroupDenseUnits, self).__init__(**kw)
      self.__check_units(units)
      self.__check_out_units(out_units)
      self.units      = units # number of units on group 
   ...
   def __check_units(self, units):
      assert (units > 1), "'units' should be more that 1"
      assert (isinstance(units, int)), "'units' should be 'int'"

   def __check_out_units(self, units):
      assert (units > 1), "'output units' should be more that 1"
      assert (isinstance(units, int)), "'output units' should be 'int'"
   ...
   def _build_shape(self, input_shape):
      ...
      # in  shape -> B, **, channels
      # out shape -> B, **, group_size, units
      self.reshape_in  = layers.Reshape(shape, name="reshape_in")
      shape = (*slice_shape, group_size*self.out_units)
      # in  shape -> B, **, group_size, units
      # out shape -> B, **, group_size*out_units
      self.reshape_out = layers.Reshape(shape, name="reshape_out")

   def _build_kernel_seq(self, input_shape):
      channels_size = input_shape[-1]
      group_size    = channels_size//self.units
      w = self.add_weight(shape=(group_size, self.units, self.out_units),
   ...
   def build(self, input_shape):
      ...
      # 
      self._build_kernel_seq(input_shape)
      # built flag
      self.built = True
