...
class DenseSoftmaxInceptionDropout(layers.Layer):
    def __init__(self, units, dropout=0.1, **kw):
        self.units   = units
        self.dropout = dropout
        self.__init_from_config(kw)
        super(DenseSoftmaxInceptionDropout, self).__init__(**kw)
    ...
    def __init_from_config(self, kw):
        ...
        if (config is not None):
            ...
            self.dense0 = layers.Dense(self.units, activation="softmax", use_bias=False, name="Dense0")
        config = kw.pop("Dense1", None)
        if (config is not None):
            self.dense1 = layers.Dense(**config)
        else:
            self.dense1 = layers.Dense(self.units, activation="softmax", use_bias=False, name="Dense1")
        config = kw.pop("Dense2", None)
        if (config is not None):
            self.dense2 = layers.Dense(**config)
        else:
            self.dense2 = layers.Dense(self.units, activation="softmax", use_bias=False, name="Dense2")
