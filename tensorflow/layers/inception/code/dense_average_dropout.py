...
class DenseAverageDropout(layers.Layer):
    def __init__(self, units, size, dropout=0.1, **kw):
        self.units   = units
        self.size    = size
        self.dropout = dropout
        self.__init_from_config(kw)
        super(DenseAverageDropout, self).__init__(**kw)
    ...
    def call(self, inputs, training=True):
        ...
        if (training == True):
            x_out = tf.nn.dropout(x_out, self.dropout, noise_shape=None, seed=None, name=None)
